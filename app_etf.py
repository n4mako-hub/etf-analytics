...
if not os.path.exists(CONFIG_FILE):
    st.error(f"File di configurazione `{CONFIG_FILE}` non trovato. Crea il file `config.yaml` prima di eseguire l'app.")
    st.stop()
...
# File di dati separati e dinamici per ogni singolo utente
WATCHLIST_FILE = os.path.join(BASE_DIR, f"watchlist_{username}.json")
SETTINGS_FILE = os.path.join(BASE_DIR, f"settings_{username}.json")
...
def load_watchlist():
    if os.path.exists(WATCHLIST_FILE):
        try:
            ...
            st.sidebar.error(f"Errore nella lettura di {os.path.basename(WATCHLIST_FILE)} ({e}). Uso la watchlist di default.")
...
def load_settings():
    if os.path.exists(SETTINGS_FILE):
        try:
            ...
            st.sidebar.error(f"Errore nella lettura di {os.path.basename(SETTINGS_FILE)} ({e}). Uso le impostazioni di default.")
...
st.sidebar.caption(f"Utente connesso: **{st.session_state.get('name', username)}**")
st.sidebar.markdown("---")
...
def render_dashboard(df_metrics, df_prices, df_ohlc, key_prefix, section_label):
            ...
            if candlestick_names:
                ...
                with st.expander("Indicatori Tecnici Avanzati (SMA, Bollinger, Fibonacci, RSI/MACD, Colori)", expanded=False):
                    ...
                    show_sma_cross = st.checkbox(
                        ...
                        help="Segna sul grafico i punti in cui la SMA più breve attiva incrocia quella più lunga attiva.",
                    ...
                    with col_vol1:
                        show_volume = st.checkbox(
                            ...
                            help="Mostra un istogramma dei volumi scambiati sotto il grafico prezzi.",
                            key=f"{key_prefix}_show_volume"
                        )
                    with col_vol2:
                        show_vwap = st.checkbox(
                            "VWAP", value=False,
                            help="Volume Weighted Average Price: prezzo medio ponderato per i volumi scambiati.",
    ...
    with tab5:
        ...
        * **Cos'è**: La SMA (Simple Moving Average) è la media aritmetica del prezzo di chiusura sugli ultimi N giorni. Appiattisce le oscillazioni giornaliere e mostra la tendenza di fondo.
        * **Come si usano in questa app**: Nel pannello "Indicatori Tecnici Avanzati" del grafico a candele puoi attivare fino a 3 SMA indipendenti.
        * **Come valutarla**: La SMA 200 è quella più usata per giudicare il trend di fondo di lungo periodo.

        ---

        ### 6bis. Incroci tra SMA — Golden Cross / Death Cross
        * **Golden Cross** (incrocio al rialzo, marcato ▲ verde): SMA breve supera dal basso verso l'alto quella lunga.
        * **Death Cross** (incrocio al ribasso, marcato ▼ rosso): SMA breve scende sotto quella lunga.

        ---

        ### 7. Bande di Bollinger & Ritracciamenti di Fibonacci
        * **Bollinger**: Bande attorno a una media mobile a 20 giorni. Banda superiore ≈ possibile ipercomprato; banda inferiore ≈ possibile ipervenduto.
        * **Fibonacci**: Livelli percentuali (23.6%, 38.2%, 50%, 61.8%) usati come possibili zone di supporto o resistenza.

        ---

        ### 8. RSI & MACD
        * **RSI**: Oscillatore di momentum (0-100). >70 = ipercomprato, <30 = ipervenduto.
        * **MACD**: Mostra la relazione tra due medie mobili esponenziali (12 e 26 giorni).

        ---

        ### 9. Volumi di Scambio
        * Mostrano l'intensità e la partecipazione del mercato a un movimento di prezzo.

        ---

        ### 10. VWAP (Volume Weighted Average Price)
        * Prezzo medio scambiato nel periodo, ponderato per i volumi.

        ---

        ### 11. Gestione Carichi e Prezzo Medio di Carico
        * Calcola il prezzo medio di carico ponderato per tutti gli acquisti registrati.

        ---

        ### 12. Zoom e Navigazione del Grafico
        * Pulsanti temporali e zoom manuale (+ / − o rotellina del mouse) per ispezionare il grafico.

        ---

        ### 13. Aggiornamento Dati vs Modifiche alla Tabella
        * Le modifiche in tabella possono essere salvate subito o applicate al prossimo aggiornamento prezzi da Yahoo Finance.

        ---

        ### 14. Personalizzazione dei Colori
        * Personalizzazione di candele, medie mobili e indicatori tramite color picker.

        ---

        ### 15. Storno dai Massimi (Drawdown) & Strategia Pullback
        * Filtra gli strumenti in trend rialzista ma stornati dai propri massimi.
        """)
    ...
    with tab6:
        ...
        st.markdown(
            ...
            "Le risposte sono generate da <b>Google Gemini</b>: serve una tua chiave API gratuita di Google AI Studio.</small>",
            unsafe_allow_html=True
        )
        ...
        with st.expander("Configurazione chiave API Gemini", expanded=not bool(st.session_state.gemini_api_key)):
            st.markdown(
                ...
                "<a href='https://aistudio.google.com/apikey' target='_blank'>Google AI Studio</a>.</small>",
                unsafe_allow_html=True
            )
            ...
            custom_model_input = st.text_input(
                "Oppure specifica manualmente un altro modello Gemini (opzionale)",
            ...
            remember_gemini_key = st.checkbox(
                "Ricorda questa chiave anche ai prossimi avvii",
            ...
            if st.button("Salva configurazione", key=f"{key_prefix}_save_gemini_cfg"):
                ...
                if remember_gemini_key:
                    ...
                    save_settings(current_settings_gemini)
                    st.success("Configurazione aggiornata (chiave non salvata su disco).")

        GEMINI_SYSTEM_CONTEXT = (
            "Sei un assistente integrato in un'app Streamlit italiana di analisi ETF e gestione portafoglio. "
            "Rispondi sempre in italiano, in modo chiaro, sintetico e pratico."
        )
                        ...
                        try:
                            ...
                            if gemini_resp.status_code == 200:
                                ...
                                if gemini_candidates and "content" in gemini_candidates[0] and "parts" in gemini_candidates[0]["content"]:
                                    ...
                                    gemini_answer = "Nessuna risposta ricevuta da Gemini."
                            elif gemini_resp.status_code == 400:
                                gemini_answer = "Richiesta non valida: controlla chiave API e modello."
                            elif gemini_resp.status_code == 403:
                                gemini_answer = "Accesso negato: chiave API non valida."
                            elif gemini_resp.status_code == 429:
                                gemini_answer = "Superato il limite di richieste. Riprova tra qualche minuto."
                            else:
                                gemini_answer = f"Errore nella chiamata a Gemini ({gemini_resp.status_code}): {gemini_resp.text[:200]}"
                        except Exception as e:
                            gemini_answer = f"Errore imprevisto: {e}"
...
if st.session_state.watchlist:
    ...
    if st.session_state.last_data_update:
        st.caption(f"Ultimo aggiornamento dati: {st.session_state.last_data_update} — le modifiche in tabella si applicano cliccando 'Aggiorna Dati' (o 'Salva Modifiche Tabella').")
